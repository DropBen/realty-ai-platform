...
- Added and locally executed 20-case offline evaluation, isolated backup/restore and 3-user mixed load baseline. PostgreSQL recovery and load succeeded in CI run 34668452691; results are in docs/evidence. PDF parsing now runs with a deadline and Linux resource limits; account deletion preserves enrolled MFA.
- Local full suite: 110 passed, one PostgreSQL-only skip. Final CI and the completion report are the authoritative final handoff evidence; earlier counts above are historical.
